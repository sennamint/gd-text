...
class Box
{
    ...
    public function draw($text)
    {
        ...
        foreach ($lines as $line) {
            ...
            /* Mod Versatil: Fix Error on empty lines */
            if (!strlen($line))
            {
                $line = " ";
            }
            ...
            if ($line && $this->backgroundColor) {
                ...
                $this->drawFilledRectangle(
                    $xMOD - 10,
                    $this->box['y'] + $yAlign + ($n * $lineHeightPx) + ($lineHeightPx - $backgroundHeight) + (1 - $this->lineHeight) * 13 * (1 / 50 * $this->fontSize) - 6,
                    $boxWidth + 20,
                    $backgroundHeight + 10,
            ...
            /* Mod: Versatil */
            $boxBottom = $this->box['y'] + $yAlign + ($n * $lineHeightPx) + $lineHeightPx;
    ...
    protected function drawInternal($x, $y, Color $color, $text)
    {
        /*
        * Mod: Versatil
        * Correct horizontal pixels offset
        */

        $imCheck = imagecreatetruecolor($this->box['width'] + 50, $this->box['height'] + 50);

        $boundingsCheck = imagefttext(
            $imCheck,
